...
더블 탭 드래그는 명시적인 사용자 옵션으로 제공하며 기본값은 OFF입니다.
...
- 선택 기능인 더블 탭 드래그는 켜져 있고 두 번째 탭이 제한 시간/거리 안에서 유지될 때만 시작합니다.
...
| 단계 | 사용자 액션 | 시스템 액션 |
|---|---|---|
| 1 | `더블탭 Drag` 활성화 | 설정을 저장. |
| 2 | 한 번 탭한 뒤 짧은 시간 안에 다시 탭하고 유지 | left-button-down 전송. |
| 3 | 한 손가락 이동 | 왼쪽 버튼 bit가 설정된 이동 리포트 전송. |
| 4 | 손가락을 뗌 | all-buttons-up 전송. |
...
7. 더블 탭 드래그는 fresh install 후 OFF 상태이며, 옵션을 켠 뒤에만 동작합니다.
