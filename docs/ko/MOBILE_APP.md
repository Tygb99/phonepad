...
GitHub Actions는 이제 push, pull request, 수동 실행에서 debug APK를 빌드합니다. `v*` 태그 빌드는 debug 라벨 APK와 checksum을 GitHub Releases에 첨부합니다.
...
5. 오른쪽 터치패드 모드를 사용합니다. `스크롤 ↑` 또는 `스크롤 ↓`를 누르고 있으면 계속 스크롤됩니다.
