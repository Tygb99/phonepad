...
- 새 PC 흐름: 새 bonded host가 앱 복귀 후 전환 대상이 되고, `호스트 연결/전환`을 눌렀을 때 HID 연결 시도가 진행되는지 확인합니다.
- 멀티 페어링: 이전/다음은 전환 대상만 바꾸고, 실제 전환은 `호스트 연결/전환`에서만 일어나는지 확인합니다.
- Windows 재페어링 초기화: Windows에서 PhonePad를 삭제하고, 앱에서 남아 있는 Windows 호스트를 선택한 뒤 `Android 페어링 삭제`를 누르고 `새 PC 연결`로 다시 페어링합니다.
...
- Windows 실패 로그: Windows에서 연결 중에 머물거나 연결 안 됨이 되면 logcat의 `PhonePad` `host_diag`, `defer_switch_*`, `connect_after_switch_*`, `connect_request_*`, `connect_timeout_*` 라인을 수집합니다.

## 호스트 한영 / 가이드 패널 스모크 체크

- Android 12 이하: 앱이 시작 시 crash 없이 열린다.
- Android 13 이상: 시스템 back으로 연결 drawer가 닫힌다.
- Bluetooth 꺼짐: 앱이 명확한 Bluetooth 상태를 표시하고 crash하지 않는다.
- Bluetooth 켜짐: HID 세션이 등록되고 PC 후보 목록이 유지된다.
- 연결 상태에서 백그라운드 이동: 활성 mouse button은 release되고 HID 세션은 유지된다.
- Windows 11: 한영 전환은 기본값에서 LANG1을 보내고, 해당 preset 선택 시 Right Alt를 보낸다.
- macOS: 한영 전환은 staged Control + Space chord를 보낸다.
- Manifest: `INTERNET` 권한이 계속 없어야 한다.
